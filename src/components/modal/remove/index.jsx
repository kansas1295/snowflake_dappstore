...
function Remove({ id, isOpen, title, toggle }) {
  ...
  const { dispatch, dapps } = user;

  const closeIcon = (
    <IoIosClose className="remove__close-icon" onClick={toggle} />
  );
  ...
  return (
    <Modal isOpen={isOpen} toggle={toggle}>
      ...
      >
        ...
        <span className="remove__title">Confirm Removal</span>
      </ModalHeader>
      <ModalBody className="remove__body">
        ...
        <Row className="pb-4">
          <Col>
            <p>
              You are about to remove <strong>{title}</strong> from your EIN.
              Please confirm below. Your purchases are tied to your EIN. You can
              always add it for free in the future after you remove it.
            </p>
          </Col>
        </Row>
        <Row className="pb-1">
          <Col>
            <TransactionButton
              ...
              sendAction={() => removeResolver(web3.library, web3.account, id)}
